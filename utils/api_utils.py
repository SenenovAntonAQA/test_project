...
def log_response(func):
    def _log_response(*args, **kwargs) -> requests.Response:
        ...
        body = json.dumps(response.json(), indent=2) if JsonUtils.is_json(
...
class ApiUtils:
    ...
    def post(self, endpoint_url, data=None, json=None, **kwargs):
        response = self.session.post(self.url + endpoint_url,
                                     data,
                                     json,
                                     **kwargs)
        return response
    ...
    def put(self, endpoint_url, data=None, **kwargs):
        response = self.session.put(self.url + endpoint_url,
                                    data=data,
                                    **kwargs)
        return response

    @log_response
    def update(self, endpoint_url, json=None, **kwargs):
        response = self.session.put(self.url + endpoint_url,
                                    json=json,
                                    **kwargs)
        return response
